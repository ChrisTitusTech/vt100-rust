...
impl FixtureScreen {
    ...
    #[allow(dead_code)]
    pub fn from_screen(screen: &vt100_ctt::Screen) -> Self {
...
{
    let s = match color {
        vt100_ctt::Color::Default => unreachable!(),
        vt100_ctt::Color::Idx(n) => format!("{n}"),
        vt100_ctt::Color::Rgb(r, g, b) => format!("#{r:02x}{g:02x}{b:02x}"),
    };
...
fn serialize_mouse_protocol_mode<S>(
    mode: &vt100_ctt::MouseProtocolMode,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let s = match mode {
        vt100_ctt::MouseProtocolMode::None => "none",
        vt100_ctt::MouseProtocolMode::Press => "press",
        vt100_ctt::MouseProtocolMode::PressRelease => "press_release",
        vt100_ctt::MouseProtocolMode::ButtonMotion => "button_motion",
        vt100_ctt::MouseProtocolMode::AnyMotion => "any_motion",
    };
    serializer.serialize_str(s)
}
...
fn serialize_mouse_protocol_encoding<S>(
    encoding: &vt100_ctt::MouseProtocolEncoding,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let s = match encoding {
        vt100_ctt::MouseProtocolEncoding::Default => "default",
        vt100_ctt::MouseProtocolEncoding::Utf8 => "utf8",
        vt100_ctt::MouseProtocolEncoding::Sgr => "sgr",
    };
    serializer.serialize_str(s)
}
